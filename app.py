...
try:
    ...
    exit(1)
# =======================================================
# FIREBASE CONNECTION SETUP - CORREÇÃO: CONFIGURAÇÃO FIXA
# =======================================================
db = None
try:
    # Configuração fixa com suas credenciais do "ura-dashboard"
    firebase_config = {
    ...
    # Inicializa Firebase com a configuração fixa
    cred = credentials.ApplicationDefault()  # Para Render, use variável de ambiente ou arquivo
    firebase_admin.initialize_app(cred, {
        'projectId': firebase_config['projectId']
    })
    db = firestore.client()
    print("Conexão com o Firebase estabelecida com sucesso usando configuração fixa.")
except Exception as e:
    print(f"Erro ao inicializar o Firebase: {e}")
    db = None  # Garante que db seja None se falhar
# Arquivos de áudio
...
def handle_gather():
    ...
    if not lead_telefone:
        ...
        response.say("Desculpe, não conseguimos identificar a campanha. Encerrando a chamada.", voice="Vitoria", language="pt-BR")
